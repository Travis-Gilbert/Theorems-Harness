...
Theorize is the divergent half of construction: it builds the space of alternatives that planning will chart and selection will choose from. In construction nobody is sovereign, contest is the normal mode, and a node from a human, a model, or a detector is distinguished by its evidence class, never by its author's rank. Divergence here is a scheduled state with a closing gate, not a personality trait: the deliverable is a destination plus the options that survived contact, handed to /planning-theorem. It is not brainstorming theater, and it is not a permanent loop.
