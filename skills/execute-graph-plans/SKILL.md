...
Done when your brief holds the occupied node's **blueprint**, the **interfaces** of adjacent nodes, **gists** of the remaining path, and the **dependents** line — and nothing beyond that. No non-adjacent blueprint enters a brief.
...
On a fault, the five-rung **repair ladder** runs with proof-carrying transitions: read the record → bounded local repair with a hypothesis → layer diagnosis → peer ask → rewrite proposal or escalation. Every rung below the resolving one is attempted or carries an explicit inapplicability reason; each attempt persists with its outcome.
...
4. **User** — through the **interrupt-economics** gate, which refuses unless rungs one through three were attempted or carry an explicit inapplicability reason. ACP permission requests are the surface of this rung only.
...
Three prohibitions that resist positive phrasing, each paired with its alternative.
...
Authoritative contracts: SPEC-THEOREM-GRAPH-TRAVERSAL-1.0 (S2 surface, S3 occupancy and handoff, S4 traverse atomic and continuation, S5 next-moves, S6 dependents, S7 membrane, S8 resolve ladder, S10 render, S11 ACP driver), SPEC-THEOREM-EDGE-LEARNING-1.0 (S2 ontology, S3 seal proposals, S4 engagement gate, S6 retrieval scoring, S9 the discriminating tests, S10 wire shape), SPEC-THEOREM-SPECIALIST-DELEGATION-1.0 (S3 delegation, S8 central mode). Ground any further signatures in these specs, not in prose summaries of them.
