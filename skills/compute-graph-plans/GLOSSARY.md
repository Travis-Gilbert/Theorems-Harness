...
Terms are grouped by axis: **The Binding** (how the doctrine touches a surface), **The Board** (what a plan is made of), **Motion** (how a head moves), **The Membrane** (how context is assembled), **Learning** (what crossing an edge deposits), **The Algebra** (the fold and evidence vocabulary), **Blocking** (what to do when stuck), and **Copresence** (how heads share a workspace).
...
The verification node attached to a work node, rendered dashed in the projection. Its presence is checked at charting: a work node without one has no path to discharge. Its oracle is fixed across every band of the work node's **attempt policy**: escalation changes strategy and head, never what passing means.

_Avoid_: test, check, validation step

### Obligation

An authoritative requirement compiled from an acceptance criterion. A node's obligations gate its **merge path**: the **gate** holds until each has replayable discharge evidence carrying an **evidence grade** the claim requires. Obligations are what make **fixpoint** unfakeable, since a head cannot assert its way past them.
...
Replayable discharge evidence: a proof command's output, a verify submission, a transition event, a `DischargeReceipt`. The unit the **gate** counts, graded by **evidence grade** — and two grades need not be comparable, so the gate treats "not ordered" as a real case. Refusals persist receipts too — there are no silent no-ops.
...
The one thing charting may declare is the **user-held information** category: a node may state that it requires a specific fact only the user holds, naming the fact. That is a declaration about the world, not a governance choice, and it is the only planned human contact after the **authority grant** is granted.
...
What the charting gate grants: a trust tier, allowed external effects, live provider authority, and protected state. It grants authority, never quantity — no token budget, no money, no wall clock. Inside the grant the run is silent, because the gate was the human contact and everything after it is exception-only.

Charting proposes the grant's defaults beside the ranked opening moves, so the grant edits a proposal rather than filling a blank.

The grant is why charting ends at a gate at all. A plan approved without one has no boundary between autonomy and babysitting, and the run drifts toward asking.

_Avoid_: allowance, quota, limits

## Motion

Five stateful operations — the four board motions, expressed natively as forms and available as tool actions on other surfaces (SPEC-THEOREM-GRAPH-TRAVERSAL-1.0 S2), plus **shed**, the window motion.
...
`(park node :reason why)`. Release without traversing, writing a block record. The honest terminal state when the reason names a condition the graph can witness: a missing credential, an unmet dependency, a verify failing at its last band, an external wait. Resumable, where a hollow completion is not. A full **window** is not a block; it is a **shed**, not a park.
...
`(shed :to graph)`. The window motion: converting load-bearing findings into durable entries before compaction reduces them to an unlawful summary. Distinguished from the four board motions, which change graph state; shed changes only which head can afford to continue. A window composes as a transition and has no lawful summary, so the shed fraction is the lossless fraction and everything else is approximated.

_Avoid_: flush, dump, save, checkpoint the context


### Fill Line

Roughly 45% of the window, the point at which shedding is still affordable. Not a limit and not a budget clock: crossing it is a checkpoint, not a terminal state. It sits early because the flush itself costs window, so headroom for durability is reserved before it is needed.

_Avoid_: threshold, cap, context budget
...
Also: a shed record that restates the session instead of naming what a future head would believe differently. Self-punishing in the same way, since it is never retrieved and decays.
...
The substrate's fold and evidence vocabulary, per the published architecture. Where a term here disagrees with the crate, the crate governs.


### Evidence Grade

`EvidenceGrade`: attested or independently verified. A grade with a named incomparability case — the two variants deliberately do not implement `Ord`, so "these two receipts are not ordered" is a real answer, not a lower-than. `VerificationOutcome` is the separate token — unverified, supported, or refuted — and a refutation is an outcome, never the bottom grade. `VerificationState` is the historic wire token; its `display_rank` is presentation order only, never a lattice operation. The gate compares grades, and treats "not ordered" as a real case.

_Avoid_: score, trust level, rank


### Retraction

`BoundedReplay`, declared by all four folds. No fold claims an inverse. A falsified assumption produces an exact interval `(checkpoint.through, T]` from the greatest compatible checkpoint strictly earlier than the affected position, or from `LogPosition::ZERO` when none qualifies. `SupportCounted` is earned after a fixed hundred-entry window records two distinct accepted retraction request ids, emits `BackfillPending`, and passes the resident-memory budget. Nothing approximates — this is the mechanical form of what `replan_subtree` does in prose.

_Avoid_: rollback, undo, revert


### Fold

A fold composes as a transition, not a snapshot. Ordered segments compose under `SegmentTransition`; a `SegmentSummaryDescriptor` is optional and none of the four current folds declares one. Two checkpoints over unrelated prefixes do not merge — the same law **shed** rests on.

_Avoid_: snapshot, merge, diff


### Semiring

Scoped, never used as a general term. `MathematicalSemiring` requires the complete law profile; `AlgebraClass::GraphBlasOperatorPair` is the weaker API construct; and the legacy probability token resolves to `IndependentNoisyOrAlgebra`, whose `AlgebraClass` variant is `IndependentNoisyOr`.

_Avoid_: semiring (unqualified)
...
Two categories reach the user, and no others:
