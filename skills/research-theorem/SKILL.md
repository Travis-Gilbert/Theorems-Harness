...
2. **Offload to the graph as you go**: `encode` the load-bearing findings with their sources, so the information belongs to every session, not this one. Sessions never hand off to each other; they offload to the graph. Done when a cold head could recall the findings without this session's context.
