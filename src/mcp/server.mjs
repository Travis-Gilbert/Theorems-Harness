...
export function toolsList() {
  return [
    ...
    {
      name: "plan",
      description: "Create and operate durable graph-backed Plans through the native MCP endpoint.",
      inputSchema: {
        type: "object",
        properties: {
          action: {
          ...
          target_status: {
            type: "string",
            enum: ["pending", "claimed", "patch_proposed", "verifying", "done", "failed"],
          },
          query: {
            type: "string",
            enum: [
          ...
          reason: { type: "string" },
          enable_import: { type: "boolean", default: false },
          legacy_checklist: { type: "object" },
          ...nativeMcpConfigSchemaProperties(),
        },
      },
    },
